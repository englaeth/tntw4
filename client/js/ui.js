...
function startGameUI() {
    ...
    if (tgPlayerName && tgPlayerName !== "Guest") {
        // Create game instance ONLY when starting solo game
        if (!gameInstance) {
            gameInstance = new Game("solo");
        } else {
            gameInstance.gameMode = "solo"; // Ensure mode is correct
        }
        // Hide menus and show game canvas BEFORE starting
        hideAllScreens();
        if(gameContainer) gameContainer.style.display = "block";
        gameInstance.start(tgPlayerName); // Start the solo game
    } else {
...
// Function to add coins and update UI
let currentCoins = 0; // Initialize coin balance

function addCoins(amount) {
    currentCoins += amount;
    updateCoinDisplay();
    // Here you might want to save the coin balance (e.g., using localStorage or sending to server)
    console.log(`Added ${amount} coins. Total: ${currentCoins}`);
}
...
document.addEventListener('DOMContentLoaded', () => {
    // Load coins from storage if available, otherwise start at 0
    // currentCoins = parseInt(localStorage.getItem('playerCoins')) || 0;
    updateCoinDisplay(); 
});
