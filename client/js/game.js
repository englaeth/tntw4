...
class Game {
        ...
        for (let i = this.poisons.length - 1; i >= 0; i--) {
            ...
            if (distance < this.player.size + GAME_SETTINGS.POISON_SIZE) {
                ...
                if (poison.isPowerup) {
                    ...
                    // Regular poison increases size
                    this.player.size += 0.5;
                    
                    // Check win condition (smaller is better)
                    if (this.player.size <= GAME_SETTINGS.WIN_SIZE) {
                        this.handleWin();
                    }
                }
    ...
    handleWin() {
        ...
        if (this.gameMode === "solo") {
            showSystemPopup(translations.youWin[currentLang] || "You Win!", false, 0, "win");
    ...
    handleMultiplayerGameOver(winnerId, players) {
        ...
        if (winnerId === socket.id) {
            ...
            showSystemPopup(`${winnerName} ${translations.wins[currentLang] || "wins!"}`, false, 0, "loss");
        }
    }
